...
const Goals = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      // Update lastAddedGoal if the deleted one was the latest
      if (lastAddedGoal && lastAddedGoal.id === id) {
        const latest = updatedGoals.length > 0
          ? updatedGoals.reduce((latest, curr) =>
              new Date(curr.created_at) > new Date(latest.created_at) ? curr : latest)
          : null;
        setLastAddedGoal(latest);
      }
